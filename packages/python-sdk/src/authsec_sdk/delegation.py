...
class DelegationResponse:
    """HTTP response with the body pre-read, safe to use after the connection closes.

    Returned by :meth:`DelegationClient.request`. The underlying aiohttp
    connection is already released, so the body is buffered here.
    """
...
class DelegationClient:
    ...
    async def request(
        ...
        **kwargs,
        ...
        Returns:
            DelegationResponse with the body pre-read (``.status``, ``.headers``,
            ``.body``, ``.text()``, ``.json()``).
        """
                ...
                if resp.status == 401 and self.auto_refresh:
                    ...
                    async with session.request(
                        ...
                        return DelegationResponse(
                            retry_resp.status, dict(retry_resp.headers), retry_body
                        )

                return DelegationResponse(resp.status, dict(resp.headers), body)
