...
class MCPServer:
    ...
    def __init__(self, client_id: str, app_name: str):
        ...
        # CORS: default remains permissive for back-compat, but production
        # deployments should set AUTHSEC_CORS_ORIGINS to a comma-separated
        # allowlist (e.g. "https://app.example.com,https://admin.example.com").
        # Note: browsers reject credentials with a wildcard origin, so
        # allow_credentials is only enabled for an explicit allowlist.
        _cors_env = os.environ.get("AUTHSEC_CORS_ORIGINS", "").strip()
        _cors_origins = (
